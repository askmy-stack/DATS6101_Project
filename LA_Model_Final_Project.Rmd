...
```{r}
memory.limit(size = 8000) # Set memory limit to 8 GB (adjust if needed)
library(randomForest)
library(caret)

# Prepare the data
crime_data_rf <- crime_data %>%
  filter(year(Date_Occ) >= 2020 & year(Date_Occ) <= 2023) %>%
  mutate(
...
X <- crime_data_rf %>% select(AREA.NAME, month, day_of_week, Vict.Sex)
...
# Assuming crime_data is your preprocessed dataset
# Filter for top 3 crimes from 2020 to 2023
top_3_crimes <- crime_data %>%
  filter(year(Date_Occ) >= 2020 & year(Date_Occ) <= 2023,
         Crm.Cd %in% c(624, 510, 310))  # Vehicle Stolen, Battery - Simple Assault, Burglary

# Prepare data for Prophet
crime_ts <- top_3_crimes %>%
  group_by(Date_Occ, AREA.NAME, Crm.Cd) %>%
  summarise(y = n()) %>%
  rename(ds = Date_Occ)

# Create and fit Prophet models for each crime type and area
areas <- unique(crime_ts$AREA.NAME)
crime_types <- unique(crime_ts$Crm.Cd)
forecasts <- list()

for (area in areas) {
  for (crime in crime_types) {
    data <- crime_ts %>% 
      filter(AREA.NAME == area, Crm.Cd == crime) %>%
      select(ds, y)
    ...
    if (nrow(data) > 0) {
      ...
      forecasts[[paste(area, crime)]] <- forecast
    }
  }
}

# Example: Plot forecast for a specific area and crime type
plot(m, forecasts[["Hollywood 510"]])
```

```{r}

plot(m, forecasts[["Pacific 624"]])
```

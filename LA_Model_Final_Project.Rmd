...
crime_data <- crime_data %>% filter(LAT != 0 | LON != 0)
...
```{r}
# Check rows where LAT or LON is zero
zero_lat_lon_rows <- crime_data %>% filter(LAT == 0 | LON == 0)

# Display the number of such rows
print(paste("Number of rows where LAT or LON is zero:", nrow(zero_lat_lon_rows)))

# Display the rows (optional)
head(zero_lat_lon_rows)
```

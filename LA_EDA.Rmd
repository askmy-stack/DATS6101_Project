...
# First, filter out unknown descent values and aggregate the data
...
p2 <- ggplot(crime_analysis_filtered, 
       aes(x = AREA.NAME, 
           y = percent_change, 
           fill = factor(Vict.Descent, 
                        levels = names(descent_labels)[names(descent_labels) != "X"],
                        labels = descent_labels[names(descent_labels) != "X"]))) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),

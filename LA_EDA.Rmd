...
## 2. Visualization Crime Trends by type and area
...
## 3. Visulatization - Percent Change in Top 3 Crimes from 2020 Benchmark - Top 5 Areas
...
## 4. Year-over-Year Percentage Change in Crime Counts
...
## 1. Calculating Year-Over-Year Percentage Change:-
...
## 2. Top 3 Crimes in Areas with Highest Increases :-

```{r q10, echo=T, results='markup'}
...
1.	Central showed the most significant increase in crime rate, with a 46.18% rise, followed by Wilshire at 26% and Rampart at 27.72%. These areas have seen substantial crime growth over the past few years, indicating potential hotspots for criminal activity.
2.	Some neighborhoods, such as 77th Street and Southeast, had minimal increases, suggesting more stable crime rates or possible effectiveness in crime control efforts.
3.	The variety in percentage changes across neighborhoods highlights that crime rate increases are not uniformly distributed in Los Angeles, with certain areas experiencing much higher growth than others, signaling where law enforcement or community initiatives might prioritize resources.

## 3. Crime Change Analysis by Sex and Race :-

```{r q11, echo=T, results='markup'}
library(dplyr)
library(ggplot2)
library(lubridate)
...
1.	High Growth in Wilshire and North Hollywood: Neighborhoods like Wilshire and North Hollywood show notable increases in crime rates across several demographics, especially for victims whose descent or sex is marked as unknown (“X”).
2.	Descent-Based Trends: Certain descent groups, such as “C” (likely representing specific ethnic codes in the dataset), appear prominently in the Wilshire area, indicating targeted impacts or reporting discrepancies.
3.	Sex-Based Analysis: The data shows varied increases across different sexes in certain areas, with Wilshire and West Valley seeing substantial growth in incidents for male and unknown (X) demographics.
4.	Need for Targeted Focus: Areas with significant increases, especially Wilshire, show the importance of focused interventions in particular demographics to understand and address underlying causes.
...
```{r q12, echo=T, results='markup'}
head(crime$Date.Rptd)
```

```{r q13, echo=T, results='markup'}
library(dplyr)
library(ggplot2)
library(lubridate)
library(plotly)
...
# Question 4
How does the type or frequency of crimes in Los Angeles vary by victim descent, and are there
significant geographic patterns (based on latitude and longitude) associated with specific victim
groups?

```{r q15, echo=T, results='markup'}
library(tidyverse)
library(ggplot2)
library(sf)
library(cluster)
...
## 1. Plotting data plots for Victim descent 
```{r q17, echo=T, results='markup'}
crime_data %>%
...
## 1. Feature Selection and Data Cleaning for Weapons Used

```{r q29, echo=T, results='markup'}
la_crime <- crime %>%
  filter(!is.na(Weapon.Used.Cd))

tail(la_crime)
```
```{r q30, echo=T, results='markup'}
cols_to_remove <- c(
...
## 2. Summarization and Plots of Top 10 crimes with Weapon Usage
```{r q33, echo=T, results='markup'}
...
## 3. Total Weapons used against various victim's sex and victim's descent.
```{r q35, echo=T, results='markup'}
...
## 4. Total Crimes against Victim's Sex and Descent
```{r q37, echo=T, results='markup'}
...
```{r q40, echo=T, results='markup', warning= False}
chi_squared_result <- chisq.test(contingency_Sex)
print(chi_squared_result)
```
```{r q41, echo=T, results='markup', warning= False}
chi_squared_result1 <- chisq.test(contingency_descent)
print(chi_squared_result1)
```
This high value of chi-squared statistic in both tables suggests a considerable deviation between observed and expected frequencies, implying a potential association within the table. The extremely low p-value strongly rejects the null hypothesis of independence, indicating a statistically significant association between the variables in contingency_Sex and contingency_Descent. Hence we can accept the alternate hypothesis that they are dependent on each other and move on to further analysis.

...
# Calculate crime counts by neighborhood and year
crime_counts <- crime_data %>%
  filter(year(DATE.OCC) >= 2020) %>%
  group_by(AREA.NAME, Year = year(DATE.OCC), Crm.Cd.Desc) %>%
...
crime_change <- crime_counts %>%
  group_by(AREA.NAME, Crm.Cd.Desc) %>%
  mutate(
    benchmark_2020 = ifelse(2020 %in% Year, crime_count[Year == 2020], NA),
    percent_change = ifelse(!is.na(benchmark_2020) & benchmark_2020 > 0,
                            (crime_count - benchmark_2020) / benchmark_2020 * 100,
                            NA)
  ) %>%
  ungroup()

# Identify top neighborhoods by total crime count
top_neighborhoods <- crime_counts %>%
  group_by(AREA.NAME) %>%
  summarise(total_crimes = sum(crime_count)) %>%
  arrange(desc(total_crimes)) %>%
  head(5)

# Create visualization of crime counts by neighborhood
p1 <- ggplot(crime_counts %>% 
             filter(AREA.NAME %in% top_neighborhoods$AREA.NAME),
       aes(x = AREA.NAME, y = crime_count, fill = Crm.Cd.Desc)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Crime Counts by Neighborhood and Type (2020-Present)",
       x = "Neighborhood",
       y = "Crime Count",
       fill = "Crime Type")

# Create visualization of percent changes
p2 <- ggplot(crime_change %>% 
             filter(AREA.NAME %in% top_neighborhoods$AREA.NAME),
       aes(x = Year, y = percent_change, color = Crm.Cd.Desc)) +
  geom_line() +
  geom_point() +
  facet_wrap(~AREA.NAME, ncol = 2) +
  theme_minimal() +
  labs(title = "Percent Change in Crime Types from 2020 Baseline",
       x = "Year",
       y = "Percent Change from 2020",
       color = "Crime Type") +
  theme(legend.position = "bottom")

# Print both visualizations
print(p1)
print(p2)

# Print summary statistics
print("Top 5 Neighborhoods by Total Crime Count:")
print(top_neighborhoods)

print("Year-over-Year Changes:")
print(crime_change %>% 
        filter(AREA.NAME %in% top_neighborhoods$AREA.NAME) %>%
        arrange(AREA.NAME, Crm.Cd.Desc, Year))
